...
module.exports = function(group, element, bpmnFactory, translate) {
  ...
  //sendElementTypeToControl(element.type);
  //type:"bpmn:ServiceTask"
    if(element.type=="bpmn:Process"){
        var scope = angular.element(document.getElementById("custom-bpmnjs")).scope();
        scope.$apply(function () {
            scope.callFromProcessTask(element.businessObject.name,element.id);
        });
    }

    if(element.type=="bpmn:EndEvent"){
        if(element.businessObject.eventDefinitions!=undefined  && element.businessObject.eventDefinitions["0"].$type=="bpmn:MessageEventDefinition"){
            //element.businessObject.eventDefinitions["0"].class="com.delegates.EndTaskDelegate";
            element.businessObject.eventDefinitions["0"].topic="endTask";
            element.businessObject.eventDefinitions["0"].type='external';
        }
    }
