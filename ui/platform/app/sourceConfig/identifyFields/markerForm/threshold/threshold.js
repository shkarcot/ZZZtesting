...
angular.module('console.markerForm')
  ...
  .controller('thresholdCtrl', function (
                              $scope,$state,$rootScope,$location,
                              Upload,$stateParams,ngDialog,
                              documentService,$window,$timeout) {
      var vm = this;
      //$(".thresholdTab").height($(window).height()-170);
      $(".threshold-custom").height($(window).height()-200);
...
//           }
      ...
      vm.saveCustomThreshold =function(){
            if(vm.objOfNewThreshold.attribute==""){
                ...
                if(editIndex!=null){
                    if(vm.thresholdsData.custom[editIndex]==NaN){
