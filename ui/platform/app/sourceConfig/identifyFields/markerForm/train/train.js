...
angular.module('console.markerForm')
  ...
  .controller('trainCtrl', function ($scope,$state,$rootScope,$location,Upload,$stateParams,ngDialog,documentService) {
      var vm = this;
      ...
      $scope.uploadFiles = function (files) {
          if (files && files.length) {
              Upload.upload({
                    ...
                    file: files,
                    data:{'data':JSON.stringify({"template_id":$stateParams.id})}
              }).then(function (response) {
                         if(response.data.status=='success'){
                             $scope.getTrainDetails($stateParams.id);
                              $.UIkit.notify({
      ...
      $scope.trainigSet = function(){
          vm.sendDocObj = {};
          vm.sendDocObj.template_id = $stateParams.id;
