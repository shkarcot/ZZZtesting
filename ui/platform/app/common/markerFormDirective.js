angular.module("console.markerDirective", [])
.directive("formImage", ["$rootScope",function($rootScope) {
    return {
        restrict: "A",
        scope: {
            src: "=formSrc",
        ...
        template: '<div form-image-selector form-model="selector" id="calImage" form-aspect-ratio="aspectRatio" style="height: {{scaleValue(height, scale) + \'px\'}}; width: {{scaleValue(width, scale) + \'px\'}}"></div><div form-image-drawer form-model="drawer" ng-class=\"$root.imageBlur\" style="height: {{scaleValue(height, scale) + \'px\'}}; width: {{scaleValue(width, scale) + \'px\'}}"></div><img ng-src="{{src}}" width="{{scaleValue(width, scale)}}" height="{{scaleValue(height, scale)}}">',
        link: function(a, b) {
            function c(c) {
...
.directive("formImageDrawer",["$rootScope",function(rootScope) {
    return {
        ...
        link: function(a) {
            ...
            a.editItem = function(rect,index){
                rootScope.$broadcast("edited",rect);
            }
        }
    }
}])
.directive("formImageSelector",["$rootScope", function(rootScope) {
    function a(a) {
        var b, c = {
                top: 0,
                left: 0
    ...
    return {
        ...
        link: function(b, c) {
            ...
            function e() {
                H[0].documentElement.className = H[0].documentElement.className.replace(" form-user-select", "")
            }

            function f() {
                H[0].documentElement.className += " form-user-select"
            }
            ...
            function o(a) {
                a.stopPropagation(), f(), W = angular.element(a.target).attr("class").replace("form-drag-handle", "").replace("form-drag-line", "").trim(), T = a.pageX, U = a.pageY, V = {
            ...
            function w(a, c, e) {
                ...
                console.log("selector"+JSON.stringify(b.selector));
            ...
            var H = angular.element(document),
                I = angular.element('<div class="form-box"><div class="form-line top"></div><div class="form-line bottom"></div><div class="form-line left"></div><div class="form-line right"></div></div>'),
                J = angular.element('<div class="form-drag-line n"></div><div class="form-drag-line s"></div><div class="form-drag-line w"></div><div class="form-drag-line e"></div>'),
                K = angular.element('<div class="form-drag-handle nw"></div><div class="form-drag-handle n"></div><div class="form-drag-handle ne"></div><div class="form-drag-handle w"></div><div class="form-drag-handle e"></div><div class="form-drag-handle sw"></div><div class="form-drag-handle s"></div><div class="form-drag-handle se"></div>');
            I.append(J).append(K), c.append(I);
            var L = angular.element('<div class="form-shadow">'),
                M = angular.element('<div class="form-shadow left">'),
                N = angular.element('<div class="form-shadow center top">'),
                O = angular.element('<div class="form-shadow center bottom">'),
                P = angular.element('<div class="form-shadow right">');
