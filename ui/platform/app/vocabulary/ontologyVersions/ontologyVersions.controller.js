module.exports = ['$scope', '$sce','$state', '$rootScope', '$location','$stateParams','$http','vocabularyService','sourceService','bpmnServices',
function($scope,$sce, $state, $rootScope, $location, $stateParams, $http, vocabularyService, sourceService, bpmnServices) {

	'use strict';
	var vm = this;
	window.scrollTo(0,0);

    $scope.openInfo = function(version){
        vm.versionData = version;
        document.getElementById("infoOpen").style.width = "40%";
    };
    $scope.cancelInfo = function(){
        document.getElementById("infoOpen").style.width = "0%";
    };
    $scope.changeState = function(){
        $rootScope.activeState = 'ontologies';
        $state.go('app.vocabulary');
    }
    sourceService.gets3URL().then(function(response){
        sourceService.sets3URL(response.data.data);
    });
    vm.s3URL = '';
    vm.ontologyId = $stateParams.id;
    vm.searchFile = '';
    vm.loginData = JSON.parse(localStorage.getItem('userInfo'));
    var solution_id = '';

    vm.getOntologyInfo = function(){
        vocabularyService.getListOfOntologiesById(solution_id,vm.ontologyId).then(function (response) {
                            if(response.data.status=='success'){
                                vm.ontologyData = response.data.data;
                                vm.versionsInfo = vm.ontologyData.versions_info;
                            }else{
                                $.UIkit.notify({
    ...
    vm.changeVersion = function(version){
        vocabularyService.toggleOntologyVersion(solution_id,vm.ontologyId,{"version":version.version,"is_active":version.is_active}).then(function (response) {
                            if(response.data.status=='success'){
                               $.UIkit.notify({
                                       message : response.data.msg,
                                       status  : 'success',
    ...
    $scope.download =function(version){
        var file_path = version.file_url.split('.com/')[1];;
        vocabularyService.downloadUrl({file_path:file_path}).then(function (response) {
                            if(response.data.status=='success'){
                               if(response.data.aws_url)
                                    window.location.assign(response.data.aws_url);
                            }else{
                                $.UIkit.notify({
                                       message : response.data.msg,
                                       status  : 'danger',
